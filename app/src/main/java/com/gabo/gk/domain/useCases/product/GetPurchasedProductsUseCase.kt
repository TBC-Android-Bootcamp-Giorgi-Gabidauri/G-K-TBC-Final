...
class GetPurchasedProductsUseCase @Inject constructor(
    ...
    override suspend fun invoke(params: Unit): Resource<List<ProductModelDomain>> {
        return try {
            var purchasedProducts: List<ProductModelDomain> = emptyList()
            when (val user = getUserUseCase(auth.currentUser!!.uid)) {
                is Resource.Success -> purchasedProducts = user.data?.purchasedProducts ?: emptyList()
                is Resource.Error -> return Resource.Error(user.errorMsg!!)
            }
            if (purchasedProducts.isNotEmpty()) {
                Resource.Success(purchasedProducts)
            } else {
