...
fun UserModelDomain.toUi() =
    UserModelUi(id, uid, email, password, type, userName, profileImg, contactNumber,
        availableAmount, searchList, soldProducts.map { it.toUi() }.toMutableList(),
        purchasedProducts.map { it.toUi() }.toMutableList()
    )
...
fun UserModelUi.toDomain() =
    UserModelDomain(id, uid, email, password, type, userName, profileImg, contactNumber,
        ...
        purchasedProducts.map { it.toDomain() }.toMutableList()
    )